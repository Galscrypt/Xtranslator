    ...
    if (selectedApis.length >= 2) {
      ...
      if (result) {
        ...
        if (!qualityGateEnabled) {
          ...
          this._reportStats();
          if (CONFIG.DEBUG) {
            console.log('[Translator] OK via ' + selectedApis[0].name + ' (solo, ' + (Date.now() - t0) + 'ms)');
          }
          return finalResult;
        }
        const check = this._validateFinalTranslation(finalResult, text);
        ...
        if (CONFIG.DEBUG) console.warn('[Translator] Quality fail via ' + selectedApis[0].name + ': ' + check.reason);
