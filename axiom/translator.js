...
const _UT_STORAGE_KEY = 'axiom_unknown_terms_v1';
const _AP_STORAGE_KEY = 'axiom_api_perf_v1';
...
class TextPreprocessor {
  ...
  preprocess(text) {
    ...
    if (this._enableSlangExpansion) {
      processed = this._applyRules(processed, this._abbrRules);
      processed = this._applyRules(processed, this._slangRules);
    }
    processed = processed.replace(_PP_RT_PREFIX, (match) => {
...
class TranslationService {
  ...
  async _translateWithFallback(text, priority = false) {
    ...
    if (this._chromeAvailable) {
      ...
      if (chromeRaw) {
        ...
        if (CONFIG.DEBUG) console.warn('[AxiomTranslator] Chrome quality fail:', check.reason);
    ...
    if (available.length >= 2) {
      ...
      if (rawResult) {
        ...
        if (CONFIG.DEBUG) console.warn('[AxiomTranslator] Quality fail via ' + rawResult.api.name + ': ' + check.reason);
        ...
        for (const api of available) {
          ...
          if (!altCheck.ok) {
            if (CONFIG.DEBUG) console.warn('[AxiomTranslator] Quality fail via ' + api.name + ': ' + altCheck.reason);
          ...
          this._reportStats();
          if (CONFIG.DEBUG) {
            console.log('[AxiomTranslator] OK via ' + api.name + ' (' + (Date.now() - t0) + 'ms) after quality fallback');
          }
          return altResult;
        }
      }
    ...
    return new Promise((resolve) => {
      ...
      const fail = (api, err) => {
        ...
        if (err.name !== 'AbortError') {
          ...
          if (CONFIG.DEBUG) console.warn('[AxiomTranslator] \u2717 ' + api.name + ' \u2014 ' + err.message);
  ...
  async _tryOneApi(api, cleanText) {
    try {
      ...
      if (CONFIG.DEBUG) console.warn('[AxiomTranslator] \u2717 ' + api.name + ' (solo) \u2014 ' + err.message);
      if (err.name !== 'AbortError') {
        api.breaker.recordFailure();
        this._recordApiFailure(api.name, false);
      }
